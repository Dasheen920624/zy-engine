...
COMMENT ON COLUMN recommendation_trigger.occurred_at         IS '触发发生时间（上游声明，默认 SYSTIMESTAMP）';
...
COMMENT ON COLUMN recommendation_card.requires_physician_confirmation IS '是否需医师确认（高风险/红线卡必须为 1/TRUE）';
