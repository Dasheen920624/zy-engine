param(
  [string]$BaseUrl = "http://localhost:18081/zy-engine/api",
  [string]$Connect = $env:ZYENGINE_DB_CONNECT,
  [string]$Username = $env:ZYENGINE_DB_USERNAME,
  [string]$Password = $env:ZYENGINE_DB_PASSWORD,
...
if ([string]::IsNullOrWhiteSpace($Password)) {
  throw "Please set ZYENGINE_DB_PASSWORD before running this script."
}
