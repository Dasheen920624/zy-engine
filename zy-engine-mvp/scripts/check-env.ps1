...
$oracleReady = -not [string]::IsNullOrWhiteSpace($env:ZYENGINE_DB_URL) -and
               -not [string]::IsNullOrWhiteSpace($env:ZYENGINE_DB_USERNAME) -and
               -not [string]::IsNullOrWhiteSpace($env:ZYENGINE_DB_PASSWORD)

if ($oracleReady) {
  Write-CheckResult "Oracle Connection Config" "PASS" "URL=$($env:ZYENGINE_DB_URL)" $COLOR_OK
  
  if (Test-CommandExists "sqlplus") {
    $connTest = & sqlplus -S "$($env:ZYENGINE_DB_USERNAME)/$($env:ZYENGINE_DB_PASSWORD)@$($env:ZYENGINE_DB_URL)" "SELECT 1 FROM DUAL;" 2>&1
...
$jarPath = Join-Path $engineRoot "target\zy-engine-mvp-0.1.0-SNAPSHOT.jar"
if (Test-Path -LiteralPath $jarPath) {
  $jarInfo = Get-Item $jarPath
  $sizeMB = [math]::Round($jarInfo.Length / 1MB, 2)
  Write-CheckResult "Application JAR" "PASS" "zy-engine-mvp-0.1.0-SNAPSHOT.jar ($sizeMB MB)" $COLOR_OK
} else {
