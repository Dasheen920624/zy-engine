...
| 文件 | 用途 |
|---|---|
| `zyengine_core_ddl_with_comments.sql` | 核心建表 + 中文 `COMMENT ON`（幂等，含 `IF NOT EXISTS`）|
| `zyengine_org_context_migration.sql` | 升级老库：补齐 PE_VARIATION_RECORD / RE_RULE_EXEC_LOG / ENGINE_AUDIT_LOG 的组织字段与索引 |
...
CREATE USER zyengine WITH PASSWORD '请改成实际密码';
CREATE DATABASE zyengine
  WITH OWNER = zyengine
       ENCODING = 'UTF8'
       LC_COLLATE = 'zh_CN.UTF-8'
       LC_CTYPE = 'zh_CN.UTF-8'
       TEMPLATE = template0;
GRANT ALL PRIVILEGES ON DATABASE zyengine TO zyengine;
SQL
```
...
```bash
psql -h <host> -p 5432 -U zyengine -d zyengine \
     -v ON_ERROR_STOP=1 \
     -f zyengine_core_ddl_with_comments.sql

# 升级旧库：
psql -h <host> -p 5432 -U zyengine -d zyengine \
     -v ON_ERROR_STOP=1 \
     -f zyengine_org_context_migration.sql
```
...
spring:
  datasource:
    ...
    url: jdbc:postgresql://${ZYENGINE_DB_HOST:localhost}:${ZYENGINE_DB_PORT:5432}/${ZYENGINE_DB_NAME:zyengine}?reWriteBatchedInserts=true&prepareThreshold=0
    username: ${ZYENGINE_DB_USERNAME:zyengine}
    password: ${ZYENGINE_DB_PASSWORD:}
zyengine:
  db:
    dialect: postgres
```
...
ksql -h <host> -p 54321 -U system -d zyengine \
     -v ON_ERROR_STOP=1 \
     -f zyengine_core_ddl_with_comments.sql
```
...
spring:
  datasource:
    ...
    url: jdbc:kingbase8://<host>:54321/zyengine
```
