...
| 文件 | 用途 |
|---|---|
| `zyengine_core_ddl.sql` | 核心建表（幂等 `IF NOT EXISTS` 风格 + 索引）|
| `zyengine_comments.sql` | 中文表备注 + 字段备注（DM UTF8 库可直接写中文，不需 UNISTR）|
| `zyengine_org_context_migration.sql` | 升级老库：补齐 PE_VARIATION_RECORD / RE_RULE_EXEC_LOG / ENGINE_AUDIT_LOG 的组织字段 |
...
```powershell
$env:ZYENGINE_DB_DIALECT='dm'
$env:ZYENGINE_DB_CONNECT='192.168.4.30:5236'
...
```text
1) zyengine_core_ddl.sql
2) zyengine_org_context_migration.sql   （旧库升级才需要）
3) zyengine_comments.sql
```
