...
| CDSS-M-01 | ⬜ 待办 | 后端 `validateCards` 失败路径仍无失败审计（需 IsolatedAuditPublisher 发 FAILED）——本轮未改后端 |
| CDSS-M-02 | ⬜ 待办 | 后端仍用 `AuditEventPublisher` 而非 `IsolatedAuditPublisher`——本轮未改后端 |
| CDSS-M-03 | ⬜ 待办 | 后端无 Micrometer 采纳率/疲劳埋点——本轮未改后端 |
| CDSS-M-04 | ⬜ 待办 | `RecommendationRepositoryTest` 无跨租户负向用例——本轮未改测试 |

**复核结论**：前端 2C+2H（及一处隐藏的触发契约假闭环）已闭环，CdssFatigue 现为诚实数据驱动页；后端 4 项 Medium（失败审计/Isolated 发布器/指标/跨租户测试）属代码质量增强，未阻断验收，留作后续。
