...
当前重构不能继续把业务拆散、重命名、弱化。主线按业务域纵向推进、从登录起，但引擎能力始终是统一共享脊柱，不得按域割裂引擎实现：
...
| 硬约束 | 落地解释 | 验收方式 |
|---|---|---|
| 按业务域纵向·B0 先行 | 沿用户旅程逐域交付（登录→工作台→试点准备→临床运行→质控改进→合规运维→高级工具），每域先做无模型 B0 真实路径；引擎能力作为统一共享脊柱按需拉入，不按域割裂 | 每域 B0 主链路可验证、域级验收通过才走下一域；不出现单病种硬编码、不按域复制引擎实现 |
...
为避免“大而散”，本方案先按 [基础底座与引擎服务能力总览](MEDKERNEL_FOUNDATION_AND_SERVICES.md) 阅读：基础底座（组织、权限、契约、审计、token、菜单壳、会话）作为 D0 平台脊柱先行；引擎服务能力（知识、字典、规则、路径、推荐、评估、随访、包发布、嵌入、证据链）按“第一个用到它的域”逐域做 B0 真实；模型网关、AI 工厂、知识自动生成等整体后移第二波 AI 加深。业务服务包装溶入各域，不再后置成单独阶段。
...
本计划以“按业务域纵向推进、从登录起、两波”为执行基线。旧的“引擎优先·自底向上（E0→E6）”节奏与旧并行计划全部冻结为历史参考，不再作为当前计划。当前目标是沿真实用户旅程逐域做到“无模型也真实可用、可验证”：真实接口、真实状态、真实发布、真实审核、真实执行、真实降级和真实证据；模型/AI 整体后移第二波。

### 17.1 上线阶段

**第一波 · B0 真实纵向**（旅程顺序＝依赖顺序）：

| 域 | 目标 | 必须交付（B0 真实） | 禁止事项 |
|---|---|---|---|
| D0 登录域/平台脊柱 | 登录与登入后可导航空壳可用 | 组织/租户上下文、五维 RBAC、API 契约、审计骨干、设计 token、菜单壳、会话/退出、真实性门禁 | 只做页面壳不做运行底座 |
| D1 工作台 | 角色首屏与演示校验真实 | 角色默认视图、租户生命周期面板、多维切片、健康/演示校验 | 业务假数据看板冒充能力完成 |
| D2 试点准备 | 配置类引擎 B0 真实 | 确定性规则/路径/知识/字典引擎 + 7 步流 + 4 状态机 + 灰度/回滚 + 适配器 + 包发布 | 单病种硬编码冒充引擎；按域割裂引擎实现 |
| D3 临床运行 | 临床主链路 B0 真实 | MPI、患者路径推进、确定性 CDSS（消费 D2 已发布规则/路径）、待办/通知、随访、嵌入 | CDSS 自动写医嘱；用模型冒充 B0 |
| D4 质控改进 | 质控评估闭环 B0 真实 | 指标/评估/整改闭环、DRG/DIP 确定性、驾驶舱下钻、AI 知识审核台的“审” | 用单业务剧本代表全平台 |
| D5 合规运维 | 身份审计安全 B0 真实 | 五维权限管理、审计查询、安全基线、证据包、Provider 状态（无连接诚实显示）| 假证据/假同步 |
| D6 高级工具 | 专家工具 B0 真实 | 来源追溯、图谱投影查询、国产化自检、开发者控制台 | 把技术对象塞进客户主路径 |

**第二波 · AI 加深**（D0~D6 的 B0 全跑通后）：AI 工厂(X-AIK) → 模型网关与真实接入(X-LLM) → 首发知识资产(X-KNOWGEN) → 15 领域门面(X-DOMAIN) → 世界级+国情剩余(X-OPT)，回灌相关域做模型增强。

### 17.2 当前允许与冻结

| 类别 | 当前允许（第一波） | 当前冻结（留第二波或禁止） |
|---|---|---|
| 推进 | D0 平台脊柱 + 登录域起，按旅程逐域 B0 真实 | 跳过 B0 直接上模型/AI；未完成域级验收就跨域 |
| 后端 | 引擎 DTO、API、服务、表族、状态、审核、发布、执行、证据的 B0 确定性实现 | 单病种或单科室硬编码业务 |
| 业务 API | 各域按需落该域稳定 API；引擎作为统一共享脊柱 | 把 S0-S40 写成当前业务接口清单；按域割裂引擎 |
| 前端 | 各域页面：7 步流、状态机、六态、审核、发布、执行、证据、降级状态 | 用业务假数据看板冒充能力完成 |
| AI | 仅诚实降级状态（B0/MODEL_DISABLED）；模型增强属第二波 | 第一波业务代码直接调用模型或 Dify |
| 测试 | 每域 B0 主链路 E2E、API 契约、状态机、5 方言、降级、安全边界 | 用单个业务剧本代表全平台 |
...
当前台账按业务域纵向管理：第一波 D0~D6 是当前执行任务，沿旅程逐域 B0 真实、域级验收通过才走下一域；第二波 AI 加深（AI 工厂/模型/知识资产/领域门面）登记为后置任务，必须等 D0~D6 的 B0 全跑通后启动，不能提前绕过 B0（铁律 #4）。

### 18.1 当前域任务（第一波 D0~D6）

详见 [backlog](backlog.md) D0~D6 各域块。每域 = 引擎/基础任务 + 页面任务 + 域级验收。

| 域 | 输出（B0 真实） | DoD |
|---|---|---|
| D0 登录域/平台脊柱 | 组织、五维权限、API 契约、审计、token、菜单壳、会话/退出、真实性门禁、登录界面 | 按角色登入、菜单按 RBAC 呈现、各页路由可打开到六态空态、退出/会话过期生效 |
| D1 工作台 | 角色首屏、租户生命周期面板、多维切片、演示校验 | 各角色首屏真实、演示与校验可跑 |
| D2 试点准备 | 确定性规则/路径/知识/字典引擎、7 步流、状态机、灰度回滚、适配器、包发布 | 配置类端到端 B0 执行，不依赖业务 mock |
| D3 临床运行 | MPI、患者路径推进、确定性 CDSS、待办/通知、随访、嵌入 | 临床主链路 B0 跑通；CDSS 需医师确认才进病历 |
| D4 质控改进 | 指标/评估/整改闭环、DRG/DIP 确定性、驾驶舱下钻、知识审核台 | 质控评估闭环 B0 跑通、驾驶舱可下钻 |
| D5 合规运维 | 五维权限管理、审计、安全基线、证据包、Provider 状态 | 身份审计安全 B0 真实、证据可导出 |
| D6 高级工具 | 来源追溯、图谱投影查询、国产化自检、开发者控制台 | 专家工具 B0 真实、技术对象不入客户主路径 |

每域以质量基线“域级验收”为 DoD。

### 18.2 第二波 AI 加深任务

业务服务包装已溶入第一波各域（D2/D3/D4/D5），不再后置成单独阶段。第二波专指跨域共享深引擎与领域门面，详见 [backlog](backlog.md)「第二波 · AI 加深」块。

| 第二波子块 | 内容 | 启动条件 |
|---|---|---|
| AI 工厂 (X-AIK) | 来源/解析/抽取/候选生成/安全校验/审核分流/知识包生成 | D0~D6 的 B0 全跑通 |
| 模型网关与真实接入 (X-LLM) | provider 无关契约、B0/B1/B2、路由策略、数据最小化、模型评测 | AI 工厂就位 |
| 首发知识资产 (X-KNOWGEN) | 术语/说明书/指南/规则/路径/指标/随访等 14 类首发资产 | AI 工厂 + 模型可用 |
| 15 领域门面 (X-DOMAIN) | 护理/药事/报告/急重症/围术期/肿瘤/中医药等专精组合，复用统一引擎链路 | 对应域 B0 + 知识资产就位 |

第二波回灌相关域时，仍复用同一资产治理、状态机、发布与证据链路，不另起服务、不另起状态机、不另起发布流程。模型增强必须在该能力 B0 路径已验收之后叠加（铁律 #4）。

### 18.3 AI 团队交付 DoD

> 下表为单项任务 DoD；一个域的整体 DoD 见质量基线“域级验收”（域内全部任务 done + 该域 B0 主链路 E2E + T-GATE 全绿 + owner≠reviewer 签字）。
...
当前代码允许作为历史骨架存在，但不能作为业务完成证明。AI 团队领取各域（D0~D6）任务时，必须同步净化相关代码。
...
| 门禁 | 标准 |
|---|---|
| 按域纵向门禁 | 按业务域纵向推进，引擎作为统一共享脊柱不得按域割裂；每域 B0 先于模型、域级验收通过才走下一域；不得单病种硬编码或假数据闭环 |
...
本节是 v1.0 GA 业务最终落地标准。第一波 D0~D6 的 B0 全跑通、第二波 AI 加深回灌后，按此判断是否最终完成。
...
从下一次 AI 团队任务开始，按业务域纵向顺序推进（详见 [backlog](backlog.md)）：

1. **D0 登录域/平台脊柱**：组织、五维权限、API 契约、审计、token、菜单壳、会话/退出、真实性门禁、登录界面；并净化代码基线（去 mock、去裸 Map）。域级验收：按角色登入、菜单 RBAC、各页路由可打开到六态空态、退出/会话过期。
2. **D1 工作台**：角色首屏、租户生命周期面板、多维切片、演示校验。
3. **D2 试点准备**：确定性规则/路径/知识/字典引擎、7 步流、状态机、灰度回滚、适配器、包发布（D3 的料）。
4. **D3 临床运行**：MPI、患者路径推进、确定性 CDSS、待办/通知、随访、嵌入。
5. **D4 质控改进**：指标/评估/整改闭环、DRG/DIP、驾驶舱下钻、知识审核台。
6. **D5 合规运维**：五维权限管理、审计、安全基线、证据包、Provider 状态。
7. **D6 高级工具**：来源追溯、图谱投影查询、国产化自检、开发者控制台。
8. **第二波 AI 加深**：D0~D6 的 B0 全跑通后，AI 工厂 → 模型网关真实接入 → 首发知识资产 → 15 领域门面 → 世界级+国情剩余，回灌相关域。
