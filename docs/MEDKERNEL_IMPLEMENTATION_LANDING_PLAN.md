...
| 实施批次 | 领域范围 | 本批必须实现 |
|---|---|---|
| P0 共用引擎闭环 | 来源知识、标准/院内字典、规则、路径、图谱解释、辅助诊疗、评估质控、随访、护理专业、医技报告解读、床旁知识查阅 | 统一数据/API/嵌入、AI 与无模型路径、审核发布、院内同步、测试证据 |
...
| 域 | 关键 API |
|---|---|
| 组织用户 | 组织树、用户、角色、权限、身份绑定、数据权限 |
| 来源同步 | 来源登记、手动同步、定时任务、离线导入、同步日志 |
| AI 候选 | 生成任务、候选列表、自动校验、审核、驳回、修改 |
| 字典映射 | 标准字典、院内字典、映射推荐、冲突、发布 |
| 配置包 | 包创建、差异、影响分析、灰度、全量、回滚、导出 |
| 规则 | 规则 CRUD、测试、仿真、执行、解释 |
| 路径 | 模板、节点、边、患者入径、变异、出院、随访生成 |
| 专病包 | 专病档案、分型分支、关键时钟、指标绑定、仿真、组织覆盖和发布 |
| 图谱 | 图谱版本、查询、来源追溯、冲突分析 |
| CDSS | 临床事件接收、推荐生成、采纳反馈、疲劳治理 |
| 护理专业 | 护理评估、分级候选、护理计划、复评、交班协同 |
| 医技报告与知识 | 报告解读、已阅/闭环动作、现行说明书/指南/路径查阅 |
| 评估质控 | 指标、运行、结果、预警、整改闭环 |
| 随访 | 计划、任务、问卷、异常、外部同步 |
| 审计证据 | 审计查询、证据快照、验收包导出 |

### 11.3 院内系统对接

| 系统 | 输入 | 输出 |
|---|---|---|
| HIS | 医嘱、费用、药品、科室、医生、就诊 | 医嘱提醒、医保提醒、映射结果 |
| EMR | 诊断、病历、病程、出院小结 | CDSS 提醒、病案质控、路径节点 |
| LIS | 检验结果、危急值 | 检验异常提醒、路径推进 |
| PACS/RIS | 检查申请、报告 | 检查建议、结果触发 |
| 病案 | 首页、编码、DRG/DIP | 首页质控、入组率、编码建议 |
| 医保 | 结算、目录、控费规则 | 医保审核、费用预警 |
| 随访系统 | 出院患者、随访任务反馈 | 随访计划、异常回院建议 |
| 消息平台 | 待办、通知、提醒 | 医生待办、审核待办、整改待办 |

### 11.4 引擎对接能力结论与详细规范引用
...
| E2 引擎接口上线 | 十大引擎能力有稳定契约 | 上下文、事件、知识、字典、规则、路径、推荐、评估、随访、包发布、嵌入、模型网关 API | 业务模块私自定义接口 |
...
| E6 业务服务包装 | 引擎能力包装成客户业务服务 | 试点准备、临床运行、质控改进、合规运维、专业领域服务包 | 绕过引擎另起业务实现 |
...
|---|---|
| 长期分支 | 只保留 `main` |
| 工作分支 | 每个任务从 main 拉 `feat/GA-xxx-short-name`，1-3 天内合并 |
| PR base | 全部指向 main |
...
当前台账只登记“基础底座 + 引擎全能力”任务。业务服务包装在 E5 引擎全能力验收后启动。
...
| E2 引擎接口 | 标准上下文、事件、知识、字典、规则、路径、推荐、评估、随访、包发布、嵌入、模型网关 API | 有 DTO、校验、OpenAPI、权限、审计和降级定义 |
...
每个业务服务包都必须包含：角色、数据、资产、流程、页面、接口、证据和验收。业务包装只组合引擎能力，不允许另起服务、另起状态机或另起发布流程。
...
7. 引擎验收通过后，再按业务服务包装试点准备、临床运行、质控改进、合规运维、专病路径和专业协同服务包。
