...
│   └── api-examples.http
│
└── _archive/                           ← 历史归档（仅供溯源，禁止作为依据）
    ├── README.md                       归档说明
    ├── deprecated-2026-05/             19 份已废弃历史长文
    ├── design-v1-draft/                V1 草稿
    └── legacy-materials/               项目最早期资料（2025-2026 初）
```
...
```
1. 项目根 README.md                              （3 分钟，项目概览）
2. docs/README.md                                （5 分钟，本文）
3. docs/01_产品事实源.md                          （20 分钟，产品全貌）
4. docs/engineering/02_任务台账.md                （找到你的任务）
5. docs/engineering/00_总入口与AI接手导航.md      （硬门禁与 DoD）
6. 按任务类型再读：
   - 业务/剧本相关 → docs/02_场景剧本图.md
   - 前端/视觉相关 → docs/03_设计系统.md
   - 实现某个页面 → docs/04_页面规格书.md（只读你的页面那一节）
   - 不确定该做什么 → docs/05_AI实施手册.md
```
...
`_archive/` 内有三类历史资料，全部**仅供 git 历史追溯，禁止作为实施依据**：

| 子目录 | 内容 | 数量 |
|---|---|---|
| `deprecated-2026-05/` | V2 之前的 19 份历史长文（产品化方案、九大产品能力、AI 复刻蓝图等） | 19 |
| `design-v1-draft/` | V1 设计草稿（1145 行单文件） | 1 |
| `legacy-materials/` | 项目最早期产品方案与演示原型 | 11 |

每份历史文档头部都已加废弃 banner，明确指向当前金本位。详细对照表见 [_archive/README.md](_archive/README.md)。
...
- V2 文档之间冲突 → 以 [01_产品事实源.md](01_产品事实源.md) 为准
- V2 与历史文档冲突 → 永远以 V2 为准
- 新增产品需求 → 先更新 01_产品事实源.md → 传导到 02/04/05
- 历史文档（`_archive/`）只读，不允许新建
- 禁止"先实现后补设计"
...
| 误区 | 正确做法 |
|---|---|
| 看到 `_archive/` 里有相关文档就引用 | `_archive/` 内全部失效，永远以 docs/01-05 为准 |
