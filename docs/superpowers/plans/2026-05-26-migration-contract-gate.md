...
新增测试类，使用 `Files.list` 读取 `src/main/resources/db/migration/{dialect}`，使用不区分大小写的正则提取 `CREATE TABLE`、`CREATE INDEX` 和 `CONSTRAINT` 名称，建立以下测试：
...
在同一测试类中按 `CREATE TABLE ... );` 表块检查租户字段、可变实体四列审计字段，以及状态/版本实体的关键字段：
...
提交中文 commit，推送 `codex/ga-eng-base-05-migration-guard`，创建中文 PR；等待远端检查全部通过后合并到远程 `main`，确认 `origin/main` 含合并提交并删除远程功能分支。
