...
- **匹配与去重物理算法**：引入标准的 LCS 最长公共子序列和 Levenshtein 编辑距离用于相似术语推荐；利用真实的 SHA-256 哈希计算在 `KnowledgeIdentityService` 对指南片段物理指纹去重。
...
- [ ] `TerminologyService` LCS（长公共子序列）及编辑距离匹配算法实现 (B2)
