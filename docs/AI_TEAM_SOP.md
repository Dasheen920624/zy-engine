...
> 上游依赖：[`AI_CHARTER.md`](AI_CHARTER.md)（What 必读规则）+ [`PRODUCT_ARCHITECTURE_FINAL.md`](PRODUCT_ARCHITECTURE_FINAL.md)（Why 决策依据）
...
| **高级 AI**（PR-V3-* 主体） | Sonnet 4.7 / GPT-4.1 / Codex 同级 | 整页面实现 / Controller + Service + Repository 三层 / 跨前后端 PR | 起 ADR（必须找架构师 AI） |
| **中级 AI**（PR-V3-* 子任务） | Sonnet 3.7 / GPT-4o / CodeBuddy | 单组件 / 单 API 接口 / 单元测试补齐 / 文档同步 | 改架构 / 改菜单结构 / 改 tokens.css |
...
```
[1. 自评] 我能领什么级别？→ 看 §1
[2. 看板] 打开 docs/engineering/02_任务台账.md → 找 status=TODO 且 owner=空 的最高优先级任务
[3. 预检] 跑 scripts/verify-task-prereq.ps1 -TaskId PR-V3-XX -Level senior
       ├─ 通过 → 进 [4]
       └─ 失败 → 看输出原因（依赖未 DONE / develop 不健康 / 能力不匹配）→ 换任务或修依赖
[4. 锁文件] 创建 ai-dev-input/10_task_claims/active/PR-V3-XX_<myId>_<timestamp>.md
       内含：write_scope（独占文件清单）/ read_scope / 预计 commit 数 / 预计完成时间 / 自评级别
[5. 必读] 严格按以下顺序读 4 份文档（10 分钟）：
       a) docs/AI_CHARTER.md（5 分钟）
       b) docs/PRODUCT_ARCHITECTURE_FINAL.md §1-§5（5 分钟）
       c) docs/05_AI实施手册.md 或 docs/v0.3-DEMO-REDESIGN.md 对应 PR 卡片
       d) docs/engineering/reference-implementations/<对应样板>
[6. 编码] 在独占文件清单内改代码；任何对清单外文件的需求 → STOP 重新拆任务
[7. 自测] 后端：cd medkernel-mvp && mvn -q compile && mvn -q test
       前端：cd frontend && npm run typecheck && npm run lint && npm test -- --run && npm run build
       任何一项 FAIL → 不许进 [8]
[8. 提交] 跑 scripts/verify-pr.ps1 -TaskId PR-V3-XX
       FAIL=0 → 进 [9]
       FAIL>0 → 修到 0
[9. PR] git commit + git push origin HEAD:develop
       commit message 格式：「PR-V3-XX: <动词> <对象>」（中文，≤ 70 字）
[10. Review] 创建 ai-dev-input/11_ai_reviews/pending/PR-V3-XX_<myId>.md
...
```bash
# AI-A 启动前必跑：
grep -r "<my-target-file>" ai-dev-input/10_task_claims/active/
# 如果有任何 write_scope 包含此文件 → 冲突，不许启动
```
...
| **跨 PR 依赖冲突**：A 改了 B 依赖的 API 形状 | A 必须先在 [`docs/engineering/02_任务台账.md`](engineering/02_任务台账.md) 该任务行加 `breaks: [PR-V3-XX]`；B 收到通知后 rebase + 适配 |
...
每个 PR-V3-* 必须满足以下 8 项 DoD（Definition of Done）：
...
- [ ] 涉及登录 → 满足 v0.3-DEMO-REDESIGN §2.1 12 条
- [ ] 涉及患者数据 → 字段最小化 + 脱敏
...
## 演示
- [ ] 在 ai-dev-input/13_feature_acceptance/ 创建 FA-PR-V3-XX-*.md，含 3+ 张截图（前端 PR）
...
| 新增国情合规能力 | `v0.3-DEMO-REDESIGN.md §2` 标 ✅ + `COMP-001_合规基线与证据包.md` 补证据 |
...
main ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ ← 永远稳定，每个 commit 是 release tag
        ↑↑                ↑↑                ↑↑
  发布 PR (人工)    发布 PR (人工)    发布 PR (人工)
        │                 │                 │
develop ━━┻━━━━━━━━━━━━━━━┻━━━━━━━━━━━━━━━━━┻━━━━━━━━━━━━━━ ← AI 都推这里
          ↑↑↑↑↑           ↑↑↑↑↑              
   ai/PR-V3-01 PR    ai/PR-V3-02 PR    ai/PR-V3-03 PR  ...
       │                 │                 │
...
| `develop` → `main` 的发布 PR 只能由**人工**发起 | 项目规约 |
| AI 任务分支命名 `ai/PR-V3-XX/<slug>` | `verify-pr.ps1` 加分支名 grep |
...
git worktree add -b ai/PR-V3-01/adapter-hub ../worktrees/pr-v3-01-adapter-hub develop
...
git branch -D ai/PR-V3-01/adapter-hub
git worktree prune
```
...
| commit message 写「fix bug」 | 后续无法定位是哪个任务 | verify-pr 检查 commit message 必须含 `PR-V3-XX` |
...
□ [1 分钟] 读 docs/PRODUCT_ARCHITECTURE_FINAL.md §1.2（确认任务对应的菜单/路由/Controller）
□ [1 分钟] 读 docs/engineering/02_任务台账.md 找到 PR-V3-XX 行（确认 owner=空 status=TODO）

5 分钟后：
□ 跑 scripts/verify-task-prereq.ps1 -TaskId PR-V3-XX -Level <senior/middle/junior>
□ 创建 active claim
□ 开始编码
```
