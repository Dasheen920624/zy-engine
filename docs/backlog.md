...
>
> 🗂️ wave2 **分 4 子块、多 PR 增量交付**（[wave2 域简报](cards/wave2/_brief.md) + [卡索引](cards/_index.md)）：**X-LLM 模型网关 11 卡已建**（批 1：API-12 · LLM-01~08 · OPT-06/09）；X-AIK / X-KNOWGEN / X-DOMAIN 待建。已建卡 ID = 卡名（`cards/wave2/<ID>.md`），合同级功能/契约/11 视角/验收以卡为准；下表「状态」仍以**实际实现**为准（建卡 ≠ 任务 done）。
