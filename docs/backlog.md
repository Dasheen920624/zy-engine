...
> 🗂️ 本域 **30 张施工卡已建**（23 ID + 7 页面，B1-B6 全建）：[D2 域简报](cards/D2/_brief.md) + [卡索引 · D2 全卡目录](cards/_index.md)；已建卡 ID = 卡名（`cards/D2/<ID>.md`），合同级功能/契约/11 视角/验收以卡为准。页面卡 ID 由占位 `D2-PAGE-*` 实化（IMPL-01 客户实施向导 / TENANT-01 租户开通 / CFGPKG-01 配置包中心 / DICTMAP-01 字典映射 / ADAPTER-01 适配器中心；规则库=RULE-01、路径配置=PATH-01 含页）。下表 ID/范围以卡为准。
