...
| D1-PAGE-工作台 | 工作台页面真实化 | 角色默认视图 + 租户生命周期面板（宪法 §6.4）+ 多维治理切片 + 本周建议动作；每页必交见质量基线 | — | pending |
| D1-PAGE-演示校验 | 演示与校验页真实化 | 演示模式 + 健康/可演示校验；每页必交见质量基线 | — | pending |
| **D1-验收** | **工作台域级验收** | 各角色首屏真实可看 + 演示与校验可跑 | — | pending |

**小计**：1 项（原 ID）+ 2 页面 + 1 域级验收
