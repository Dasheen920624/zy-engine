...
5. 同时复制同名 `.sha256` 摘要文件；恢复前必须由 `restore.sh` 自动校验通过。
6. 记录当前 Git commit、Dify 版本和 `deploy/docker/dify/compose.lock.yml` 摘要。
...
数据库恢复会覆盖当前 MedKernel PostgreSQL 数据，应只在确认需要回退数据状态且备份 `.sha256` 摘要校验通过时执行。
...
- `backup.sh` 生成的备份文件路径和 `.sha256` 校验摘要。
- 当前 Git commit。
