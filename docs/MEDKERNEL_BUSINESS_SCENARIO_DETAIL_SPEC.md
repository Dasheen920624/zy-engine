...
业务统计、工作台面板、证据导出和服务包验收统一使用 **多维治理切片**。旧三维示例只能作为最小示例，不能再写成系统上限。

| 维度组 | 可选维度 | 用途 |
|---|---|---|
| 组织层级 | 租户、集团、医院、院区、社区服务点、科室、病区、医生团队、角色 | 覆盖范围、灰度发布、权限和责任归属 |
| 临床对象 | 患者、就诊、诊断、病种、专病包、路径模板、患者路径、随访计划 | 临床运行、专病路径和随访闭环 |
| 服务能力 | 接入、知识、字典、规则、路径、推荐、评估、随访、包发布、嵌入、模型网关、大列表、审计 | 判断引擎能力是否上线、可用、降级或待验收 |
| 资产版本 | 知识包、配置包、规则集、路径包、指标集、问卷、模型策略 | 审核、发布、回滚、替换和历史重放 |
| 流程状态 | 租户 6 阶段、7 步流、任务状态、告警状态、整改状态、同步状态 | 指导下一步动作和验收材料生成 |
| 外部接入 | HIS、EMR、LIS、PACS、医保、病案、公卫、区域平台、Provider、Webhook | 管理第三方接入、字段映射、健康检查、回调和补偿 |
| 风险证据 | P0-P3、数据质量、降级状态、traceId、审计事件、证据快照、导出任务 | 支撑医疗安全、质量整改、交付验收和监管证据 |

默认页面只能展示当前角色最需要的少量维度；复杂组合必须进入高级筛选、详情抽屉、专家模式或异步导出。任何新增业务细节都必须先归入上述维度组，再决定是否需要新的页面、服务包或 API 归类。
...
本表只是接口地图。未标“后续 E6”的路径是当前底座或引擎 API；标“后续 E6”的路径只作为业务包装设计起点，不作为当前落地承诺。
...
| 医生反馈 | `POST /api/v1/engine/recommendations/cards/{cardId}/feedback` | 采纳/不采纳、原因、医生 | 反馈留痕、疲劳治理输入 |
| 智能评估 | `POST /api/v1/engine/evaluations/run` | 指标集、病例和组织范围 | 评估结果、问题、整改建议 |
| 护理评估与计划 | 后续 E6：`POST /api/v1/engine/nursing/assess-and-plan` | 病情、自理能力、护理风险、治疗和路径 | 护理分级候选、风险评估、护理计划候选、复评时间 |
...
第三方对接不是第六套业务实现。所有外部系统都先进入标准上下文、临床事件、适配器、包发布、嵌入、审计和降级链，再由 E6 业务包装形成客户可见接口。

| 能力层 | 当前复用能力 | 后续 E6 包装 | 边界 |
|---|---|---|---|
| 院内系统接入 | 标准上下文、临床事件、字典映射、数据质量、适配器配置 | 接入管理、字段映射、健康检查和数据质量门面 | 不要求医院一次性标准化改造 |
| 临床工作流嵌入 | launch token、iframe/SDK/纯 API、推荐卡、反馈回传 | 医生站、护士站、质控系统和门户嵌入套件 | 不替代 HIS/EMR 页面主流程 |
| 外部事件回流 | 同步、异步、批量、回放、payload、诊断、重试、死信 | 第三方回调、业务状态回传和待办联动 | 回调必须签名、白名单、幂等 |
| 标准互操作门面 | FHIR 映射、CDS Hooks 风格事件、私有接口适配 | FHIR/CDS Hooks/SMART launch 兼容服务包 | 标准门面不替代院内私有适配 |
| 包发布与院内同步 | 包差异、灰度、全量、同步日志、回滚、离线包 | 知识包/配置包分发、同步监控、失败补偿 | 同步失败不得破坏权威版本 |
| 合规与证据交换 | 审计事件、证据快照、大列表导出、脱敏审批 | 评级证据、监管报送、科研数据服务 | 法定上报和支付结论人工确认 |
| 外部模型与工作流 | 模型能力网关、B0/B1/B2、Dify 可选同步 | Provider 管理、模型审批、调用审计 | 业务代码不得直连模型厂商或 Dify |

| 第三方系统 | 接入方向 | 复用引擎能力 | 业务输出 |
|---|---|---|---|
| HIS / 医嘱 / 费用 | 双向 | 上下文、事件、规则、推荐、字典、医保规则 | 医嘱提醒、费用预警、反馈留痕 |
| EMR / 病历 / 病程 | 双向 | 上下文、事件、规则、评估、推荐、嵌入 | 病历质控、路径节点、低打扰提醒 |
| LIS / 检验 / 危急值 | 输入为主，闭环回传 | 事件、规则、推荐、评估、报告闭环 | 异常提示、危急值处置、质控证据 |
| PACS / RIS / 病理 / 内镜 | 输入为主，闭环回传 | 事件、知识、规则、推荐、报告闭环 | 报告解读、复核候选、结果闭环 |
| 药房 / 审方 / 药事平台 | 双向 | 字典、知识、规则、推荐、评估 | 用药安全、抗菌药物治理、点评问题 |
| 护理 / 移动护理 / 交班 | 双向 | 上下文、路径、规则、推荐、随访 | 护理计划、复评、交班重点和执行反馈 |
| 手麻 / 手术室 / 输血 | 输入为主，确认回传 | 路径、规则、推荐、评估、审计 | 围术期核查、麻醉风险、用血闭环 |
| ICU / 监护 / 生命体征 | 输入为主 | 事件、规则、推荐、评估、审计 | 风险预警、升级待办、处置反馈 |
| 医保 / 病案 / DRG/DIP | 双向 | 字典、规则、评估、证据导出 | 首页质控、医保审核、整改闭环 |
| 公卫 / 院感 / 上报系统 | 输出预填，人工确认 | 事件、规则、评估、审计 | 报告卡预填、感染风险、上报证据 |
| 不良事件 / 投诉 / 安全系统 | 双向 | 事件、评估、整改、审计 | 事件分类、改进任务、效果复盘 |
| 随访 / 消息 / 通知平台 | 双向 | 随访、事件、待办、推荐、审计 | 任务分发、问卷回收、异常回院 |
| CA / 电子签名 / SSO | 输入身份，输出审计 | 安全、权限、组织上下文、审计 | 身份绑定、签署证据、权限校验 |
| 区域平台 / 医联体 / 双向转诊 | 双向 | 适配器、上下文、事件、路径、随访 | 互认提示、远程协同、转诊接续 |
| SPD / UDI / 器械耗材 | 输入为主 | 字典、知识、规则、审计 | 适用提醒、召回停用、技术准入 |
| 科研 / 伦理 / 数据平台 | 输出为主，审批回传 | 大列表、脱敏、审计、证据导出 | 脱敏队列、数据集、审批证据 |
| 电子病历评级 / 互联互通测评 | 输出为主 | 评估、审计、证据包、大列表导出 | 覆盖率、材料索引、整改任务 |
| 模型 Provider / Dify / 本地模型 | 输入输出受控 | 模型能力网关、策略、脱敏、审计 | AI 候选、解释草稿、降级状态 |

第三方对接必须同时满足：

| 约束 | 标准 |
|---|---|
| 身份 | 外部系统身份必须映射到院内用户、角色和组织范围 |
| 幂等 | 事件、批量、回调、同步和导出均有幂等键或业务去重键 |
| 安全 | 签名、白名单、最小字段、脱敏、权限和审计不可省略 |
| 降级 | 外部系统断开时，医生主流程不阻断；系统记录失败、重试和人工补录入口 |
| 证据 | 每个接入点都能导出接口日志、字段映射、数据质量、失败补偿和处置证据 |
| 边界 | 不控制医疗设备，不自动执行医嘱、输血、手术、法定上报、医保支付或病历归档 |

第三方接口对接必须同步交付接口文档，未交付不得进入联调。

| 文档项 | 必须写清 |
|---|---|
| 接入概览 | 对接系统、业务场景、接入方向、同步/异步/批量方式、触发点、责任人和环境 |
| 契约定义 | OpenAPI、事件 schema、文件/中间库格式、DTO、校验规则、枚举码表和错误码 |
| 字段映射 | 来源字段、标准上下文字段、转换规则、必填/可空、脱敏规则、质量指标和样例 |
| 鉴权安全 | 身份映射、权限范围、签名算法、证书/密钥、IP 白名单、时间戳、防重放和最小字段 |
| 幂等可靠 | 幂等键、业务去重键、重试策略、超时阈值、死信、回放、人工补偿和重复消息处理 |
| 回调通知 | 回调地址、签名头、状态机、payload、重试窗口、失败告警和外部系统确认方式 |
| 降级边界 | 外部系统不可用时的用户提示、人工补录入口、医生主流程影响和恢复动作 |
| 审计证据 | traceId、审计事件、接口日志、字段映射版本、数据质量报告、失败补偿记录和验收用例 |
| 医疗边界 | 不自动写医嘱、病历、支付、法定上报、签署、输血、手术、设备控制或归档结论 |
...
| 接口归类 | 场景表中的接口必须标明属于“当前引擎 API”“后续 E6 业务包装 API”“复用已有引擎 API”“外部系统集成 API”或“暂不设独立 API”；禁止把后续设想写成当前已落地契约 |
...
| 第三方业务接口服务包 | S2、S13、S14、S23、S33、S40 | 接入管理、字段映射、健康检查、互操作门面、回调、区域协同和证据交换 |
...
每个服务包必须保持 8 个固定部分：角色、数据、资产、流程、页面、接口归类、证据、验收。服务包不得新增独立状态机、独立模型调用、独立发布流程或独立证据格式。
...
S0-S40 是业务能力场景，不等于 41 组独立后端服务。当前 0 业务引擎阶段优先交付共享引擎和支撑 API，业务场景按下表归类；没有写独立路径的场景不是遗漏，而是复用共享引擎或等待 E6 业务包装层统一定义。

| 归类 | 说明 | 文档写法 |
|---|---|---|
| 当前引擎 API | 已纳入当前底座或引擎契约，开发可按该路径实现和验收 | 写 `API 归类`，并以“当前引擎 API”开头 |
| 后续 E6 业务包装 API | 属于业务服务包门面、编排或聚合接口，当前阶段不作为落地承诺 | 写 `API 归类`，明确“后续 E6”，并列出复用的当前引擎能力 |
| 复用已有引擎 API | 场景本身不新增接口，通过上下文、事件、规则、路径、推荐、评估、随访、包发布、审计等共享接口完成 | 写 `API 归类`，列出复用能力，不新增业务路径 |
| 外部系统集成 API | 院内 EMR/HIS/LIS/PACS/手麻/输血/区域平台等系统对接 | 写 `对接系统` 或 `集成 API`，并通过适配器、事件和审计链治理 |
| 暂不设独立 API | 纯流程、页面、制度、证据组织或人工确认能力，不需要独立后端路径 | 写 `API 归类`，说明暂不新增接口和原因 |

业务上按下列 5 组理解接口边界：

| 业务组 | 覆盖场景 | 主接口归类 | 业务解释 |
|---|---|---|---|
| A 平台治理与接入 | S0-S3、S14-S15 | 平台支撑 API + 后续 E6 业务包装 API | 做组织、接入、知识工厂、权限、验收和工作台聚合；当前不把每个管理页面都拆成独立临床引擎 |
| B 共享引擎配置 | S4-S8、S11-S13 | 当前引擎 API | 字典、规则、路径、推荐、评估、随访和包发布是当前优先落地的稳定接口 |
| C 临床运行服务 | S16-S22、S24-S25、S27-S31、S35-S39 | 复用已有引擎 API + 后续 E6 业务包装 API | 面向医生、护士、药师和专科中心的业务场景，先复用引擎能力，后续再做业务门面 |
| D 评级质控与合规证据 | S9-S10、S23、S32-S34 | 复用已有引擎 API + 后续 E6 业务包装 API | 以规则、评估、审计、证据包和大列表导出为底座，业务专题接口后置 |
| E 外部系统协同 | S2、S24-S26、S30、S40 | 外部系统集成 API + 复用已有引擎 API | 与院内系统、区域平台、设备或第三方流程对接；中枢只做适配、事件、提示、证据和闭环 |
...
| API 归类 | 后续 E6 工作台聚合 API。当前复用系统健康、审计、任务、大列表和各引擎状态；不新增独立工作台接口 |
...
| API 归类 | 平台支撑 API + 后续 E6 管理 API。租户、组织、用户和生命周期属于平台治理能力；当前按组织上下文、权限和审计契约实现 |
...
| API 归类 | 外部系统集成 API + 后续 E6 接入管理 API。院内系统通过适配器、事件、字段映射和审计链接入；当前不把接入管理拆成独立业务服务 |
...
| 禁止 | AI 候选未经审核直接生效 |
| API 归类 | 后续 E6 知识工厂业务 API + 当前知识身份能力。来源发现、候选分流和审核走知识工厂门面；当前先复用知识身份、候选分类、审核、包发布和审计能力 |
...
| API 归类 | 当前引擎 API：`/api/v1/engine/terminology/local-terms`、`/api/v1/engine/terminology/mappings`、`/api/v1/engine/terminology/conflicts` |
...
| API 归类 | 后续 E6 图谱解释 API + 复用已有引擎 API。当前先复用各引擎诊断、来源字段和审计链；图谱查询不作为当前业务接口承诺 |
...
| API 归类 | 当前引擎 API：`/api/v1/engine/recommendations/triggers`、`/api/v1/engine/recommendations/cards`、`/api/v1/engine/recommendations/cards/{cardId}/feedback` |
...
| API 归类 | 后续 E6 质控业务包装 API。当前复用上下文、规则执行、评估问题、整改任务和审计证据；不新增独立病历质控接口 |
...
| API 归类 | 后续 E6 医保病案业务包装 API。当前复用字典、规则、评估、审计和证据导出；不新增独立医保病案专题接口 |
...
| API 归类 | 当前引擎 API：`/api/v1/engine/evaluations/indicators`、`/api/v1/engine/evaluations/results`、`/api/v1/engine/evaluations/findings` |
...
| API 归类 | 当前引擎 API：`/api/v1/engine/followup/plans`、`/api/v1/engine/followup/events/report-abnormal`、`/api/v1/engine/followup/plans/generate` |
...
| API 归类 | 平台支撑 API + 后续 E6 权限管理 API。当前以安全、组织上下文和合规审计为准；权限管理门面由 E6 定义 |
...
| API 归类 | 后续 E6 验收支撑 API。当前复用接口测试、E2E 证据、审计链和大列表导出；验收门面由 E6 定义 |
...
| API 归类 | 后续 E6 辅助诊疗 API。当前复用上下文、规则、路径、推荐卡片、知识身份、评估和审计链；不把诊断/治疗建议写成当前独立接口 |
...
| API 归类 | 后续 E6 医嘱推荐 API。当前复用上下文、规则、路径、推荐卡片和审计链；不把检查检验推荐写成当前独立接口 |
...
| API 归类 | 后续 E6 药事与治疗 API。当前复用字典、规则、知识、推荐卡片和审计链；不把用药治疗建议写成当前独立接口 |
...
| API 归类 | 后续 E6 风险预警 API。当前复用事件、规则、推荐、评估和审计链；不把急危重症预警写成当前独立接口 |
...
| API 归类 | 后续 E6 护理康复宣教 API。当前复用路径、随访、知识、推荐和审计链；不新增独立护理康复宣教接口 |
...
| API 归类 | 后续 E6 院感公卫 API。当前复用事件、规则、评估、推荐卡片和审计链；不新增独立院感公卫接口 |
...
| API 归类 | 后续 E6 MDT 协同 API。当前复用上下文、知识、路径、推荐、评估和审计链；不新增独立 MDT 协同接口 |
...
| API 归类 | 后续 E6 评级支撑 API。当前复用评估、大列表、合规审计和证据包导出；不新增独立电子病历评级接口 |
...
| API 归类 | 复用已有引擎 API。当前通过上下文、事件、规则、推荐、路径推进和审计链完成；暂不设独立门急诊业务 API |
...
| API 归类 | 复用已有引擎 API。当前阶段通过上下文、事件、规则、路径、评估、随访和审计链组合实现；住院闭环门面接口待 E6 业务包装层定义 |
...
| API 归类 | 外部系统集成 API + 复用已有引擎 API。手麻、手术室和输血系统通过适配器/事件接入，当前阶段复用规则、路径、推荐、评估和审计能力；不单独定义设备控制或输血执行接口 |
...
| API 归类 | 复用已有引擎 API + 后续 E6 重症业务包装 API。当前通过上下文、事件、规则、推荐、评估和审计链实现；不定义生命支持设备控制接口 |
...
| API 归类 | 复用已有引擎 API。特殊人群作为上下文、规则条件、知识适用条件和推荐约束复用，不单独拆接口 |
...
| API 归类 | 复用已有引擎 API + 后续 E6 肿瘤业务包装 API。当前复用知识、路径、规则、推荐、评估、随访和 MDT 协同能力 |
...
| API 归类 | 外部系统集成 API + 复用已有引擎 API。基层、随访和区域平台通过适配器/事件接入；当前复用路径、随访、评估、包发布和审计能力 |
...
| API 归类 | 复用已有引擎 API + 后续 E6 药事业务包装 API。当前复用字典、知识、规则、推荐、评估和审计链 |
...
| API 归类 | 外部系统集成 API + 后续 E6 安全事件包装 API。当前复用事件、评估、整改、审计和证据包能力 |
...
| API 归类 | 外部系统集成 API + 复用已有引擎 API。耗材、UDI、物资和手术室系统通过适配器接入；当前复用字典、知识、规则和审计链 |
...
| API 归类 | 后续 E6 数据服务包装 API。当前复用大规模列表、异步导出、权限、审计、脱敏和证据链；科研接口不得绕过审批 |
...
| API 归类 | 后续 E6 护理专业 API。当前复用上下文、规则、路径、推荐、随访、评估和审计链；护理专业门面由 E6 定义 |
...
| API 归类 | 后续 E6 报告闭环 API。当前复用上下文、事件、知识、规则、推荐、评估和审计链；报告闭环门面由 E6 定义 |
...
| API 归类 | 当前知识身份 API + 后续 E6 查阅问答 API。当前复用知识身份 active/lineage、发布包和权限审计；床旁查阅门面由 E6 定义 |
...
| API 归类 | 复用已有引擎 API + 后续 E6 专业协同包装 API。当前复用上下文、量表规则、路径、随访、推荐和审计链 |
...
| API 归类 | 复用已有引擎 API + 后续 E6 中医药健康管理包装 API。当前复用知识、字典、路径、规则、随访、包发布和审计链 |
...
| API 归类 | 外部系统集成 API + 后续 E6 区域协同包装 API。当前复用适配器、上下文、事件、规则、路径、推荐、审计和证据链 |
...
| 类型 | 最小实现 |
|---|---|
| API 归类 | 后续 E6 替换分析 API：`POST /api/v1/engine/knowledge/identities/{id}/versions/{versionId}/replacement-analysis`，生成被替代资产、依赖和病例影响分析 |
| API 归类 | 后续 E6 知识工厂包装 API：`POST /api/v1/ai-candidates/classify`，按知识身份与现行权威版本分类新主题、新版、重复、旧版、冲突或撤回 |
| API 归类 | 当前知识身份 API：`POST /api/v1/engine/knowledge/identities/{identityId}/versions/{versionId}/activate`，审核后原子替换，并返回旧版失效与同步结果 |
| API 归类 | 当前知识身份 API：`POST /api/v1/engine/knowledge/identities/{identityId}/versions/{versionId}/withdraw`，紧急撤回并触发风险处置 |
| API 归类 | 当前知识身份 API：`GET /api/v1/engine/knowledge/identities/{id}/lineage`，展示来源、候选、审核、发布、替代和撤回链 |
...
| 领域包实现 | 矩阵内各专业领域均有实现卡、审核责任、资产 schema、API 归类/B0 路径、测试和发布证据；不得只保留名称 |
