...
- 客户可见、医学/医保/质控、数据库持久化、前端页面、发布同步、安全权限任务必须建立 `ai-dev-input/13_feature_acceptance/` 功能验收记录。
- 功能验收未达到 `GOLD` 或未明确接受剩余风险时，不得作为客户演示卖点；P0/P1/P2 开放项必须进入 `needs_optimization/` 并登记后续优化任务。
- 新增或修改表、字段、索引、约束、迁移脚本时，必须同步生产库与开发库 DDL，并补齐中文表/字段备注；公共 Controller、Service、Provider、复杂规则、发布/回滚/同步、医学/医保/质控判断必须有简洁意图注释。
...
- 表、字段、关键约束是否有中文备注；H2 开发库不得替代生产库备注。
- 复杂业务逻辑、发布/回滚/同步、医学/医保/质控判断是否有必要注释，且没有无意义逐行注释。
...
- 新增或改造客户可见功能、演示路径、专业业务闭环时，必须触发功能验收。
- 新增或改造数据库表/字段、复杂规则、发布同步、医学/医保/质控逻辑时，必须触发注释和备注审查。
...
- 表、字段、索引、约束、备注是否完整，生产库 DDL 必须含中文 `COMMENT ON` 或等价备注脚本。
- LOCAL_H2_FILE 只承担离线开发结构等价，不替代生产库 DDL、备注和 smoke 计划。
- 迁移脚本是否可重复执行。
...
- 是否有敏感信息泄漏。
- 公共 Controller、Service、Provider 是否有简洁职责或意图注释。
- 复杂规则、发布/回滚/同步、医学/医保/质控判断是否有维护者能理解的注释。
- 是否避免无意义逐行注释、过期注释和与代码矛盾的注释。
...
- 是否已在 `ai-dev-input/13_feature_acceptance/` 创建验收记录。
- 产品、架构、数据库、前端、测试、安全、医学/医保/质控等必要角色是否有结论。
- 客户验收故事线是否能实际演示，且不依赖口头说明补齐关键流程。
- 质量等级是否明确为 `GOLD`、`SILVER`、`BRONZE` 或 `REJECTED`。
- 未达 `GOLD` 的功能是否进入 `needs_optimization/` 并登记后续优化任务。
...
表/字段备注与必要代码注释已补齐：
需要功能验收时，feature_acceptance_id 已填写：
功能验收状态为 APPROVED/GOLD 或已进入 needs_optimization 并登记后续任务：
```
...
6. 运行 `check-ai-collaboration.ps1` 和 `git status -sb`。
7. 无冲突后创建并推送 claim。
8. 开发、自检、提交评审。
9. 需要时创建功能验收记录。
10. 根据质控问题整改。
11. 通过评审和必要功能验收后正式提交。
12. 归档 claim、review 和功能验收记录。
```
