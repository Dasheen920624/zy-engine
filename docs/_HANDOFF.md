...
- **状态**：分支已起、接力机制已提交；**B1 后端现状已 grep 核实**（见下）；`docs/cards/D2/_brief.md` 与 B1 四卡**待建**
...
- **下一步**（精确动作）：
  ...
  3. 回填 `_index`/`_coverage-matrix`/backlog D2 段 → 提交（机制 + B1 同 PR，分支 `claude/d2-foundation`）→ 请用户合并
