...
- **剩余（待用户起）**：
  ...
  2. `_coverage-matrix` 卡级 §-锚点细化（**非轻量**，可后补）。
...
---
> 末次更新：2026-05-30 · 核心域 D0–D6 全建（#152–#162）+ 直链 sweep（#163）；GA 总验收 12 卡已建（`claude/ga-acceptance`，PR 待合，实际 pass 待 wave2）；剩 wave2（55 项）待用户起
