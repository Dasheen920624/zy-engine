...
function getQcIndicators() {
  return [
    { id: 1, name: "I类手术切口预防性抗菌药物使用率", value: 24.5, target: 30, unit: "%", status: "PASS", desc: "符合国家卫生健康委抗菌药物临床应用专项整治指标（≤30%）要求。" },
    { id: 2, name: "全院30天非计划再入院率", value: 4.8, target: 5.0, unit: "%", status: "PASS", desc: "出院质量保持高标准运行，出院随访触发机制有效阻断非正常出院。" },
    { id: 3, name: "抗菌药物使用强度 (AUD)", value: 42.1, target: 40, unit: "DDDs", status: "WARN", desc: "超出目标值（≤40 DDDs）。主要是呼吸内科与骨科抗真菌药物超频。" },
    { id: 4, name: "高风险临床路径入径率", value: 92.5, target: 90, unit: "%", status: "PASS", desc: "急重症转诊通道与临床路径引擎联合运行顺畅，入径率提升显著。" }
  ];
}
...
function getDrilldownCases() {
  return [
    { id: "case-01", patientName: "王*平", age: 62, gender: "男", recordNo: "IP-268041", docName: "李明国", issue: "预防性抗菌药物术前给药时间晚于切皮前2小时，违反围手术期规范", traceId: "tr-924081", status: "已下达整改" },
    { id: "case-02", patientName: "张*华", age: 48, gender: "女", recordNo: "IP-268042", docName: "陈伟杰", issue: "出院带药重复配置两种同类三代头孢菌素，存在超频滥用倾向", traceId: "tr-924082", status: "科室已递交整改" }
  ];
}
    ...
    >
      <div className={styles.container}>
          ...
          <div className={`${styles.card} ${styles.flexAlign}`}>
            ...
            <div>
              ...
              <div className={styles.description}>
                质量考核核心安全指标达标占比：{indicators.filter((i) => i.status === "PASS").length}/{indicators.length} 个
              </div>
            </div>
          </div>
            ...
            {indicators.map((ind) => {
              ...
              return (
                ...
                >
                  ...
                  <div className={styles.descTextMargin}>
                    {ind.desc}
                  </div>
                </div>
              );
        ...
        <div className={styles.card}>
          ...
          <div className={styles.description}>
            状态一目了然：绿色表示质量极佳，黄色表示有违规倾向或轻微变异，红色表示红线缺陷急需 PDCA 闭环整改。
          </div>
          <div className={styles.heatmapGrid}>
            {getDeptRisks().map((dept) => (
              <div
                key={dept.id}
                className={`${styles.heatmapCell} ${getHeatmapClass(dept.level)}`}
              >
          ...
          <div className={styles.modalOverlay}>
            <div className={styles.modalContent}>
              ...
              <div className={styles.modalBodyText}>
                <div className={styles.marginBottom8}><strong>当前计算值：</strong> {selectedIndicator.value} {selectedIndicator.unit} (达标基线: {selectedIndicator.target} {selectedIndicator.unit})</div>
                <div className={styles.marginTop10}><strong>评估分析：</strong> {selectedIndicator.desc}</div>
              </div>
                  ...
                  <tbody>
                    {getDrilldownCases().map((c: CaseItem) => (
                      <tr key={c.id}>
                        ...
                        <td className={styles.fontWeight600}>{c.patientName} ({c.gender}, {c.age}岁)</td>
                        <td>{c.docName}</td>
