...
Oracle 中文表备注和字段备注推荐执行 `db/oracle/medkernel_comments_unistr.sql`，该脚本使用 `UNISTR` 写入中文，可避免客户端字符集不一致导致备注乱码。
...
```text
GET http://localhost:18080/medkernel/api/health
```
...
```text
GET http://localhost:18082/medkernel/api/health
GET http://localhost:18082/medkernel/api/system/providers
```
...
```powershell
$env:MEDKERNEL_GRAPH_ENABLED='true'
$env:MEDKERNEL_GRAPH_URI='bolt://localhost:7687'
$env:MEDKERNEL_GRAPH_USERNAME='neo4j'
$env:MEDKERNEL_GRAPH_PASSWORD='图谱密码'
$env:MEDKERNEL_GRAPH_DATABASE='neo4j'

$env:MEDKERNEL_DIFY_ENABLED='true'
$env:MEDKERNEL_DIFY_BASE_URL='https://dify.example.com'
$env:MEDKERNEL_DIFY_API_KEY='Dify应用API密钥'
```
...
```text
GET http://localhost:18081/medkernel/api/health
```
...
```text
db/oracle/medkernel_core_ddl_with_comments.sql
```
...
脚本会创建核心表、索引、表备注和字段备注，并执行 `medkernel_org_context_migration.sql` 补齐已有库的组织上下文字段和索引。表已存在时会跳过建表，不会删除已有数据；迁移脚本会把 `PE_PATIENT_INSTANCE` 活动实例唯一约束升级为 `tenant_id + org_code + encounter_id + pathway_code + status`。为避免中文备注乱码，脚本会在建表和迁移后再次执行 `medkernel_comments_unistr.sql` 覆盖写入中文备注。
...
```text
POST /medkernel/api/patient-pathways/candidates
...
```text
POST /medkernel/api/patient-pathways/admit
```
...
```text
POST /medkernel/api/patient-pathways/{instanceId}/nodes/AMI_CHEST_PAIN_IDENTIFY/complete
```

节点任务与变异记录：

```text
GET  /medkernel/api/patient-pathways/{instanceId}
GET  /medkernel/api/patient-pathways/{instanceId}/nodes/{nodeCode}
POST /medkernel/api/patient-pathways/{instanceId}/nodes/{nodeCode}/tasks/{taskCode}/complete
POST /medkernel/api/patient-pathways/{instanceId}/nodes/{nodeCode}/tasks/{taskCode}/skip
POST /medkernel/api/patient-pathways/{instanceId}/variations
GET  /medkernel/api/pathways
GET  /medkernel/api/pathways/{pathwayCode}?versionNo=1.0.0
GET  /medkernel/api/pathways/{pathwayCode}/diff?from=1.0.0&to=draft
POST /medkernel/api/pathways/{pathwayCode}/rollback
POST /medkernel/api/graph/disease-candidates
POST /medkernel/api/graph/evidence
POST /medkernel/api/graph/versions
GET  /medkernel/api/graph/versions
GET  /medkernel/api/graph/versions/{graphVersion}
POST /medkernel/api/graph/versions/{graphVersion}/activate
POST /medkernel/api/graph/evidences
GET  /medkernel/api/graph/evidences?graphVersion=&targetCode=&targetType=&evidenceType=&limit=100
GET  /medkernel/api/graph/evidences/{evidenceId}
POST /medkernel/api/graph/nodes
GET  /medkernel/api/graph/nodes?graphVersion=&type=&limit=200
POST /medkernel/api/graph/edges
GET  /medkernel/api/graph/edges?graphVersion=&fromCode=&toCode=&relationType=&limit=200
POST /medkernel/api/terminology/normalize
POST /medkernel/api/terminology/mappings
GET  /medkernel/api/terminology/mappings
GET  /medkernel/api/terminology/mappings/{sourceSystem}/{sourceCode}?conceptType=DIAGNOSIS
POST /medkernel/api/adapters/query
POST /medkernel/api/adapters/definitions
GET  /medkernel/api/adapters/definitions
GET  /medkernel/api/adapters/definitions/{adapterCode}/{queryCode}
POST /medkernel/api/dify/workflows/run
POST /medkernel/api/dify/workflows
GET  /medkernel/api/dify/workflows
GET  /medkernel/api/dify/workflows/stats?workflowCode=&workflowVersion=&status=&provider=&patientId=&encounterId=&limit=500
GET  /medkernel/api/dify/workflows/{workflowCode}?workflowVersion=1.0.0
POST /medkernel/api/config-packages
POST /medkernel/api/config-packages/import
GET  /medkernel/api/config-packages?tenantId=&assetType=&status=&scopeLevel=&scopeCode=
GET  /medkernel/api/config-packages/{packageCode}/{packageVersion}?tenantId=
GET  /medkernel/api/config-packages/{packageCode}/{packageVersion}/review?tenantId=
POST /medkernel/api/config-packages/{packageCode}/{packageVersion}/review?tenantId=
POST /medkernel/api/config-packages/{packageCode}/{packageVersion}/publish?tenantId=
POST /medkernel/api/config-packages/{packageCode}/{packageVersion}/export?tenantId=
GET  /medkernel/api/system/providers
GET  /medkernel/api/system/org-context
POST /medkernel/api/organizations
GET  /medkernel/api/organizations?tenantId=&level=&parentLevel=&parentCode=&status=&limit=
GET  /medkernel/api/organizations/tree?tenantId=&rootLevel=&rootCode=
GET  /medkernel/api/organizations/{level}/{code}?tenantId=
GET  /medkernel/api/rules?tenantId=&hospitalCode=&scopeLevel=&scopeCode=
GET  /medkernel/api/rules/exec-logs?ruleCode=&traceId=&patientId=&encounterId=&resultStatus=&hit=&tenantId=&hospitalCode=&scopeLevel=&scopeCode=&limit=100
GET  /medkernel/api/rules/exec-logs/summary?ruleCode=&traceId=&patientId=&encounterId=&resultStatus=&hit=&tenantId=&hospitalCode=&scopeLevel=&scopeCode=
GET  /medkernel/api/rules/exec-logs/{logId}
GET  /medkernel/api/rules/packages/{packageCode}/review?packageVersion=2026.05
POST /medkernel/api/rules/packages/{packageCode}/publish
GET  /medkernel/api/pathway-variations?pathwayCode=&patientId=&encounterId=&variationType=&nodeCode=&instanceId=&tenantId=&hospitalCode=&scopeLevel=&scopeCode=&limit=100
GET  /medkernel/api/pathway-variations/summary?pathwayCode=&patientId=&encounterId=&variationType=&nodeCode=&instanceId=&tenantId=&hospitalCode=&scopeLevel=&scopeCode=
GET  /medkernel/api/pathway-instances?pathwayCode=&status=&patientId=&encounterId=&currentNodeCode=&tenantId=&hospitalCode=&scopeLevel=&scopeCode=&limit=100
GET  /medkernel/api/pathway-instances/summary?pathwayCode=&status=&patientId=&encounterId=&currentNodeCode=&tenantId=&hospitalCode=&scopeLevel=&scopeCode=
GET  /medkernel/api/pathway-instances/node-completion?pathwayCode=&status=&patientId=&encounterId=&tenantId=&hospitalCode=&scopeLevel=&scopeCode=
GET  /medkernel/api/pathway-instances/node-stay-duration?pathwayCode=&status=&patientId=&encounterId=&tenantId=&hospitalCode=&scopeLevel=&scopeCode=
GET  /medkernel/api/quality/metrics?pathwayCode=&status=&patientId=&encounterId=&currentNodeCode=&workflowCode=&tenantId=&hospitalCode=&scopeLevel=&scopeCode=
GET  /medkernel/api/audit-logs?engineType=&actionType=&targetType=&targetCode=&patientId=&encounterId=&traceId=&tenantId=&hospitalCode=&scopeLevel=&scopeCode=&limit=100
GET  /medkernel/api/audit-logs/summary?engineType=&actionType=&targetType=&targetCode=&patientId=&encounterId=&traceId=&tenantId=&hospitalCode=&scopeLevel=&scopeCode=
```
