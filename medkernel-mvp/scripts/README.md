...
已有库会额外执行 `db/oracle/medkernel_org_context_migration.sql`，补齐 `PE_VARIATION_RECORD`、`RE_RULE_EXEC_LOG`、`ENGINE_AUDIT_LOG` 的组织字段与索引，并把 `PE_PATIENT_INSTANCE` 活动实例唯一约束升级为 `tenant_id + org_code + encounter_id + pathway_code + status`。
...
本地库文件位于 `medkernel-mvp/data/local-db/`，已被 `.gitignore` 忽略。H2 只作为 AI/离线开发 Provider，属于开发库；Oracle 是当前生产权威库，达梦、PostgreSQL、KingbaseES 是生产兼容交付库。任何 DDL 变更必须同步维护 Oracle、达梦、PostgreSQL-Kingbase 和 LOCAL_H2_FILE 结构文件。
...
```text
http://localhost:18081/medkernel/api/health
```
