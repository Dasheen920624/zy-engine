-- SEC-010: 审计防篡改、密钥轮换和安全基线 DDL (DM 达梦)
...
-- 3. 为现有审计表添加防篡改字段
ALTER TABLE engine_audit_log ADD record_hash VARCHAR(128);
ALTER TABLE engine_audit_log ADD chain_hash VARCHAR(128);

ALTER TABLE sec_auth_audit_log ADD record_hash VARCHAR(128);
ALTER TABLE sec_auth_audit_log ADD chain_hash VARCHAR(128);

ALTER TABLE sec_sso_audit_log ADD record_hash VARCHAR(128);
ALTER TABLE sec_sso_audit_log ADD chain_hash VARCHAR(128);
