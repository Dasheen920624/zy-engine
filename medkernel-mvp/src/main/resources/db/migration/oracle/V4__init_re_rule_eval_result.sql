-- RE_RULE_EVAL_RESULT 表 DDL (Oracle)
-- 规则评估结果表，支持跨实例查询和持久化

-- 创建表
CREATE TABLE re_rule_eval_result (
  id NUMBER(20) PRIMARY KEY,
  eval_id VARCHAR2(64) NOT NULL,
...
-- 创建索引
CREATE INDEX idx_re_eval_result_eval_id ON re_rule_eval_result (eval_id);
CREATE INDEX idx_re_eval_result_rule ON re_rule_eval_result (rule_code, rule_version);
CREATE INDEX idx_re_eval_result_patient ON re_rule_eval_result (patient_id, encounter_id);
CREATE INDEX idx_re_eval_result_org ON re_rule_eval_result (tenant_id, hospital_code, scope_level, scope_code);
CREATE INDEX idx_re_eval_result_time ON re_rule_eval_result (created_time);
