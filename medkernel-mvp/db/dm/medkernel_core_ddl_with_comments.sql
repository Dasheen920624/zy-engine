-- 达梦 DM 8 / DM 7 核心 DDL（含中文备注）
-- 设计原则：与 db/oracle 等价；ID 应用层生成；JSON 用 CLOB；时间用 TIMESTAMP
-- 字符集要求：库 CHARSET=UTF8（dminit 时 CHARSET=1）
--
-- 执行方式：通过 disql 或 DM 管理工具按顺序执行：
--   1) 本文件
--   2) medkernel_org_context_migration.sql（旧库升级时）
--
-- 中文备注：DM UTF8 库可直接写中文，无需 UNISTR。
...
CREATE TABLE IF NOT EXISTS pe_patient_node_state (
  ...
  timeout_flag TINYINT DEFAULT 0 NOT NULL,
  created_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL
);
