-- SEC-002 数据权限 DDL (Oracle)
-- 数据权限策略表 + 数据权限分配表

-- 数据权限策略表
CREATE TABLE sec_data_permission_policy (
  id NUMBER(19) PRIMARY KEY,
  tenant_id NUMBER(19) NOT NULL,
  policy_code VARCHAR2(64) NOT NULL,
...
-- 数据权限分配表
CREATE TABLE sec_data_permission_assignment (
  id NUMBER(19) PRIMARY KEY,
  tenant_id NUMBER(19) NOT NULL,
  assignment_code VARCHAR2(64) NOT NULL,
  principal_type VARCHAR2(32) NOT NULL,
  principal_code VARCHAR2(64) NOT NULL,
  principal_name VARCHAR2(200),
  policy_id NUMBER(19) NOT NULL,
  policy_code VARCHAR2(64),
...
-- 索引
CREATE INDEX idx_sec_dp_assignment_principal ON sec_data_permission_assignment (tenant_id, principal_type, principal_code);
CREATE INDEX idx_sec_dp_assignment_resource ON sec_data_permission_assignment (tenant_id, resource_type, effect);
CREATE INDEX idx_sec_dp_policy_tenant_type ON sec_data_permission_policy (tenant_id, policy_type, enabled);
